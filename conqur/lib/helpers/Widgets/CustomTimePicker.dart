...
typedef GestureDayTapCallback = void Function(DayPicker);
...
const ShapeBorder _kHourShape =
    RoundedRectangleBorder(borderRadius: _kHourBorderRadius);
const ShapeBorder _kMinuteShape =
    RoundedRectangleBorder(borderRadius: _kMinuteBorderRadius);

const Color _accentColor = Color.fromRGBO(159, 219, 167, 1);
    ...
    switch (orientation) {
      case Orientation.portrait:
        ...
        controls = Column(
          children: <Widget>[
            SizedBox(
              height: _kTimePickerDayControlHeight,
              child: Row(
                children: <Widget>[
                  // if (!use24HourDials &&
                  //     timeOfDayFormat ==
...
class _HourMinuteControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle style = isSelected
        ? kLThreeTextStyle(Colors.black, FontWeight.bold)
        : kLThreeTextStyle(
            Colors.black,
          );

    final ShapeBorder shape = isHour ? _kHourShape : _kMinuteShape;
...
class _DayControl extends StatelessWidget {
  const _DayControl({
  ...
  final DayPicker selectedDay;
  final Orientation orientation;
  final ValueChanged<DayPicker> onChanged;

  void _togglePeriod() {
    if (selectedDay == DayPicker.today) {
      onChanged(DayPicker.yesterday);
    } else {
      onChanged(DayPicker.today);
    }
  }

  void _setYesterday(BuildContext context) {
    if (selectedDay == DayPicker.yesterday) {
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final bool yesterdaySelected = selectedDay == DayPicker.yesterday;
    final Set<MaterialState> yesterdayStates = yesterdaySelected
        ? <MaterialState>{MaterialState.selected}
        : <MaterialState>{};
    final bool todaySelected = !yesterdaySelected;
    final Set<MaterialState> todayStates = todaySelected
        ? <MaterialState>{MaterialState.selected}
        : <MaterialState>{};
    final TextStyle textStyle = timePickerTheme.dayPeriodTextStyle ??
        Theme.of(context).textTheme.subtitle1!;
    final TextStyle yesterdayStyle = yesterdaySelected
        ? kLThreeTextStyle(Colors.black)
        : kLThreeTextStyle(Colors.white);
    final TextStyle todayStyle = !yesterdaySelected
        ? kLThreeTextStyle(Colors.black)
        : kLThreeTextStyle(colorScheme.onSurface.withOpacity(0.60));
    ...
    final double buttonTextScaleFactor =
        math.min(MediaQuery.of(context).textScaleFactor, 1.5);

    final Widget yesterdayButton = Material(
      color: MaterialStateProperty.resolveAs(backgroundColor, yesterdayStates),
      child: InkWell(
        onTap: Feedback.wrapForTap(() => _setYesterday(context), context),
        child: Semantics(
          checked: yesterdaySelected,
    ...
    final Widget todayButton = Material(
      color: MaterialStateProperty.resolveAs(backgroundColor, todayStates),
      child: InkWell(
        onTap: Feedback.wrapForTap(() => _setToday(context), context),
        child: Semantics(
    ...
    switch (orientation) {
      case Orientation.portrait:
        const double width = _kTimePickerDayControlWidth;
        result = SizedBox(
          width: width,
          height: _kTimePickerDayControlHeight,
          child: Material(
            ...
            child: Neumorphic(
              ...
              child: Row(
                children: <Widget>[
                  Expanded(child: yesterdayButton),
                  // Container(
...
class _DayPeriodControl extends StatelessWidget {
  ...
  void _togglePeriod() {
    ...
    title = selectedTime.period == DayPeriod.am ? "AM" : "PM";
    onChanged(newTime);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final Set<MaterialState> pmStates = pmSelected
        ? <MaterialState>{MaterialState.selected}
        : <MaterialState>{};
    final TextStyle amStyle = kLThreeTextStyle(textColor, FontWeight.bold);
    OutlinedBorder shape = timePickerTheme.dayPeriodShape ??
        const RoundedRectangleBorder(borderRadius: _kDayPeriodBorderRadius);
    ...
    final Widget amButton = Material(
      ...
      child: InkWell(
        onTap: Feedback.wrapForTap(() => _setTime(context), context),
        child: Semantics(
          checked: amSelected,
          inMutuallyExclusiveGroup: true,
          button: true,
...
class _DialPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const double focusedRadius = _labelPadding - 14.0;
    canvas.drawCircle(centerPoint, 5.0, selectorPaint);
    canvas.drawCircle(focusedPoint, focusedRadius, selectorPaint);
    selectorPaint.strokeWidth = 7.0;
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  TimeOfDay _getTimeForTheta(double theta, {bool roundMinutes = true}) {
  ...
  TimeOfDay _notifyOnChangedIfNeeded({bool roundMinutes = true}) {
  ...
  void _updateThetaForPan({bool roundMinutes = true}) {
    setState(() {
      ...
      <_TappableLabel>[
        for (final TimeOfDay timeOfDay in _twentyFourHours)
          _buildTappableLabel(
            ...
            color,
            timeOfDay.hour,
            localizations.formatHour(timeOfDay, alwaysUse24HourFormat: false),
            () {
  ...
  Widget build(BuildContext context) {
    ...
    final Color backgroundColor =
        pickerTheme.dialBackgroundColor ?? Colors.grey.withOpacity(0.12);
    final Color accentColor = pickerTheme.dialHandColor ?? _accentColor;
...
class _TimePickerInputState extends State<_TimePickerInput>
  ...
  late DayPicker _selectedDay = DayPicker.today;
...
class TimePickerDialog extends StatefulWidget {
  ...
  ///the time when the value is changed
...
class _TimePickerDialogState extends State<TimePickerDialog>
  ...
  DayPicker _selectedDay = DayPicker.today;
  ...
  void _handleDayChanged(DayPicker value) {
    setState(() {
      _selectedDay = value;
      ...
      date = DateTime(
          date.year,
          date.month,
          _selectedDay == DayPicker.today ? date.day : yesterday.day,
  ...
  Size _dialogSize(BuildContext context) {
    ...
    return Size(double.infinity, timePickerHeight * textScaleFactor);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final MediaQueryData media = MediaQuery.of(context);
    final TimeOfDayFormat timeOfDayFormat =
        localizations.timeOfDayFormat(alwaysUse24HourFormat: false);
    final bool use24HourDials = false;
    final ThemeData theme = Theme.of(context);
    ...
    switch (_entryMode.value) {
      case TimePickerEntryMode.dial:
        ...
        final Widget dayOption = _DayControl(
              ...
              children: <Widget>[
                Column(
                  ...
                  children: <Widget>[
                    ...
                    Neumorphic(child: Container(child: dayOption)),
                    dial,
                    header,
                  ],
                ),
